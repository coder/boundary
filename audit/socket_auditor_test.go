...
func TestSocketAuditor_AuditRequest_QueuesLog(t *testing.T) {
	...
	case log := <-auditor.logCh:
		...
		// Rule should not be set for allowed requests
		if httpReq.MatchedRule != "" {
...
func TestSocketAuditor_AuditRequest_DeniedIncludesRule(t *testing.T) {
	t.Parallel()
	...
	case log := <-auditor.logCh:
		...
		if httpReq.MatchedRule != "block-evil" {
			t.Errorf("expected MatchedRule=block-evil, got %s", httpReq.MatchedRule)
		}
	default:
		t.Fatal("expected log in channel, got none")
	}
